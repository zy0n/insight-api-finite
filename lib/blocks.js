  ...
  if (blockCached) {
    ...
    self.node.getBlock(hash, function(err, block) {
      ...
      self.node.services.bitcoind.getBlockHeader(hash, function(err, info) {
        ...
        self.getBlockReward(blockResult.height, function(err,reward) {
          if (err) {
            return self.common.handleErrors(err, res);
          }
...
//change this to input the current block height.
BlockController.prototype.getBlockReward = function(nHeight, cb) {
  var self = this;
  var nSubsidyHalvingInterval = 562500;
  var nBudgetPaymentsStartBlock = 81000;

  // dont need to use previous hash to get height,
  var nSubsidy = 32;
  for (var i = nSubsidyHalvingInterval; i <= nHeight; i += nSubsidyHalvingInterval) {
        nSubsidy -= nSubsidy/2.5;
  }
  // Hard fork to reduce the block reward by 10 extra percent (allowing budget/superblocks)
  var nSuperblockPart = (nHeight > nBudgetPaymentsStartBlock) ? nSubsidy/10 : 0;
