<h1 align="center">Insight-api-finite</h1>

<div align="center">
  <strong>A Finite blockchain REST and WebSocket API Service</strong>
</div>
<br />
<div align="center">
  <!-- Stability -->
  ...
  <!-- Build Status -->
  <a href="https://travis-ci.org/zy0n/insight-api-finite">
    <img src="https://img.shields.io/travis/zy0n/insight-api-finite/master.svg?style=flat-square" alt="Build Status" />
  </a>
  <!-- NPM version -->
  <a href="https://npmjs.org/package/insight-api-finite">
    <img src="https://img.shields.io/npm/v/insight-api-finite.svg?style=flat-square" alt="NPM version" />
  </a>
</div>


This is a backend-only service. If you're looking for the web frontend application, take a look at https://github.com/zy0n/insight-ui-finite.
...
- [Resources](#resources)
- [License](https://github.com/zy0n/insight-api-finite/blob/master/LICENSE)

## Getting Started

```bashl
npm install -g bitcore-node-finite@latest
...
The API endpoints will be available by default at: `http://localhost:3001/insight-api-finite/`

### Prerequisites

- [Bitcore Node Finite 3.x](https://github.com/zy0n/bitcore-node-finite)

**Note:** You can use an existing Finite data directory, however `txindex`, `addressindex`, `timestampindex` and `spentindex` needs to be set to true in `finite.conf`, as well as a few other additional fields.
...
### Block
```
  /insight-api-finite/block/[:hash]
  /insight-api-finite/block/0000000006e7b38e8ab2d351239019c01de9a148b5baef58cfe52dfd9917cedc
```

### Block Index
Get block hash by height
```
  /insight-api-finite/block-index/[:height]
  /insight-api-finite/block-index/0
```
This would return:
```
{
...
### Raw Block
```
  /insight-api-finite/rawblock/[:blockHash]
```
...
```
  /insight-api-finite/blocks?limit=3&blockDate=2017-04-22
```
...
### Transaction
```
  /insight-api-finite/tx/[:txid]
  /insight-api-finite/tx/ebdca263fe1c75c8609ce8fe3d82a320a0b3ca840f4df995883f5dab1b9ff8d9
  /insight-api-finite/rawtx/[:rawid]
  /insight-api-finite/rawtx/ebdca263fe1c75c8609ce8fe3d82a320a0b3ca840f4df995883f5dab1b9ff8d9
```

### Address
```
  /insight-api-finite/addr/[:addr][?noTxList=1][&from=&to=]
  /insight-api-finite/addr/ybi3gej7Ea1MysEYLR7UMs3rMuLJH5aVsW?noTxList=1
  /insight-api-finite/addr/yPv7h2i8v3dJjfSH4L3x91JSJszjdbsJJA?from=1000&to=2000
```

### Address Properties
```
  /insight-api-finite/addr/[:addr]/balance
  /insight-api-finite/addr/[:addr]/totalReceived
  /insight-api-finite/addr/[:addr]/totalSent
  /insight-api-finite/addr/[:addr]/unconfirmedBalance
```
The response contains the value in Satoshis.

### Unspent Outputs
```
  /insight-api-finite/addr/[:addr]/utxo
```
Sample return:
```
[
...
GET method:
```
  /insight-api-finite/addrs/[:addrs]/utxo
  /insight-api-finite/addrs/ygwNQgE5f15Ygopbs2KPRYMS4TcffqBpsz,ygw5yCtVkx3hREke4L8qDqQtnNoAiPKTSx/utxo
```

POST method:
```
  /insight-api-finite/addrs/utxo
```
...
```
  /insight-api-finite/txs/?block=HASH
  /insight-api-finite/txs/?block=000000000814dd7cf470bd835334ea6624ebf0291ea857a5ab37c65592726375
```
### Transactions by Address
```
  /insight-api-finite/txs/?address=ADDR
  /insight-api-finite/txs/?address=yWFfdp9nLUjy1kJczFhRuBMUjtTkTTiyMv
```

### Transactions for Multiple Addresses
GET method:
```
  /insight-api-finite/addrs/[:addrs]/txs[?from=&to=]
  /insight-api-finite/addrs/ygwNQgE5f15Ygopbs2KPRYMS4TcffqBpsz,ygw5yCtVkx3hREke4L8qDqQtnNoAiPKTSx/txs?from=0&to=20
```

POST method:
```
  /insight-api-finite/addrs/txs
```
...
GET method:
```
  /insight-api-finite/gobject/list/proposal
```
...
GET method:
```
  /insight-api-finite/gobject/get/[:hash]
  /insight-api-finite/gobject/get/b6af3e70c686f660541a77bc035df2e5e46841020699ce3ec8fad786f7d1aa35
```
...
### Masternodes List
```
  /insight-api-finite/masternodes/list
```
### Validate Masternode
```
  /insight-api-finite/masternodes/validate/[:payee]
  /insight-api-finite/masternodes/validate/yRuALkPpeYpTgxdNn2L5YgGktASJYDYPAo
```
...
```
  /insight-api-finite/sync
```

### Live Network P2P Data Sync Status
```
  /insight-api-finite/peer
```

### Status of the Bitcoin Network
```
  /insight-api-finite/status?q=xxx
```
...
### Utility Methods
```
  /insight-api-finite/utils/estimatefee[?nbBlocks=2]
```
...
- (Medium)[How to setup a Finite Instant-Send Transaction using Insight API?????????The comprehensive way](https://medium.com/@obusco/setup-instant-send-transaction-the-comprehensive-way-a80a8a0572e)
